...
import { userDetailStart, userDetailSuccess, userDetailFailure } from '../redux/user/userSlice'
...
const signIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const loginUrl = formData.role === "Candidate" ? "http://127.0.0.1:8000/c_login/" : "http://127.0.0.1:8000/i_login/";
    ...
    try {
      ...
      if (data.message) {
        navigate('/');
        dispatch(userDetailSuccess({ ...data, role: formData.role}))
      }
      ...
      console.log(error);
      dispatch(userDetailFailure({ message: 'Network error' }))
    }
  };
    ...
    <div className=" bg-gray-100 flex justify-center items-center p-10">
      <div className="w-4/5 bg-white rounded-lg shadow-lg flex flex-row flex-wrap">
        ...
        <div className="w-1/2 p-12">
          ...
          <p className="text-gray-600 mb-8 text-sm">Enter to get unlimited access to data & information.</p>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4 relative">
              ...
              <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" id="Password" type="password" placeholder="Enter password" onChange={handleChange} />
          ...
          <p className="text-center mt-8 text-sm text-gray-600">
            Don't have an account ? <Link to={'/signUp'} className="text-indigo-600 font-bold">Register here</Link>
          </p>
        </div>
